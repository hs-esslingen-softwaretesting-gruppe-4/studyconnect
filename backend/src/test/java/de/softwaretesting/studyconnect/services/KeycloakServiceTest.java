...
class KeycloakServiceTest {
    ...
    @Mock
    private KeycloakAdminTokenService keycloakAdminTokenService;
    ...
    void createRealm_shouldReturnTrue_whenRealmAlreadyExists() {
        ...
        Map<String, Object>[] realmsArray = new Map[] {
            Map.of("realm", TEST_REALM),
            Map.of("realm", "other-realm")
        };
    ...
    void createUserInRealm_shouldReturnTrue_whenUserCreatedSuccessfully() {
        ...
        boolean result = keycloakService.createUserInRealm("password123", "test@example.com", "John", "Doe");
    ...
    void createAdminUserInRealm_shouldReturnTrue_whenAdminCreatedSuccessfully() {
        ...
        boolean result = keycloakService.createAdminUserInRealm("admin123", "admin@example.com", "Admin", "User");
    ...
    void retrieveUserByUUID_shouldReturnUser_whenFound() {
        ...
        KeycloakUserResponseDTO expectedUser = new KeycloakUserResponseDTO(
            userId, System.currentTimeMillis(), "testuser", true, true,
            "John", "Doe", "test@example.com", null, null, null
        );
        ResponseEntity<KeycloakUserResponseDTO> response = new ResponseEntity<>(expectedUser, HttpStatus.OK);
        when(restTemplate.exchange(
    ...
    void retrieveUserByEmail_shouldReturnUser_whenFound() {
        ...
        KeycloakUserResponseDTO expectedUser = new KeycloakUserResponseDTO(
            "user-uuid", System.currentTimeMillis(), "testuser", true, true,
            "John", "Doe", email, null, null, null
        );
        KeycloakUserResponseDTO[] usersArray = new KeycloakUserResponseDTO[] { expectedUser };
        ResponseEntity<KeycloakUserResponseDTO[]> response = new ResponseEntity<>(usersArray, HttpStatus.OK);
        when(restTemplate.exchange(
    ...
    void retrieveUserByEmail_shouldReturnNull_whenNotFound() {
        ...
        ResponseEntity<KeycloakUserResponseDTO[]> response = new ResponseEntity<>(emptyArray, HttpStatus.OK);
        when(restTemplate.exchange(
            eq(TEST_KEYCLOAK_URL + "/admin/realms/" + TEST_REALM + "/users?email=" + email),
            eq(HttpMethod.GET),
            any(HttpEntity.class),
    ...
    void retrieveAllUsersInRealm_shouldReturnUsers_whenFound() {
        KeycloakUserResponseDTO user1 = new KeycloakUserResponseDTO(
            "uuid-1", System.currentTimeMillis(), "user1", true, true,
            "John", "Doe", "john@example.com", null, null, null
        );
        KeycloakUserResponseDTO user2 = new KeycloakUserResponseDTO(
            "uuid-2", System.currentTimeMillis(), "user2", true, true,
