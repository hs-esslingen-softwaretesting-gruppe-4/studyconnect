...
class KeycloakAdminTokenServiceTest {
    ...
    void init_shouldFetchToken_whenCredentialsAreConfigured() {
        ...
        when(restTemplate.postForEntity(anyString(), any(HttpEntity.class), eq(KeycloakTokenResponseDTO.class)))
    ...
    void getAccessToken_shouldFetchNewToken_whenNoTokenExists() {
        ...
        assertEquals("test-access-token", token);
        verify(restTemplate).postForEntity(anyString(), any(HttpEntity.class), eq(KeycloakTokenResponseDTO.class));
    }

    @Test
    void getAccessToken_shouldRefreshToken_whenTokenIsExpired() {
        // First, set up an expired token
        KeycloakTokenResponseDTO expiredTokenResponse = createTokenResponse(1L, 1800L); // 1 second expiry
        KeycloakTokenResponseDTO newTokenResponse = new KeycloakTokenResponseDTO(
        ...
        ReflectionTestUtils.setField(tokenService, "tokenExpiryTime", 
            java.time.Instant.now().minusSeconds(100));

        String token = tokenService.getAccessToken();

        assertEquals("new-access-token", token);
        verify(restTemplate, times(2)).postForEntity(anyString(), any(HttpEntity.class), eq(KeycloakTokenResponseDTO.class));
    }
    ...
    void scheduleTokenRefresh_shouldNotRefresh_whenTokenIsStillValid() {
        ...
        verify(restTemplate, times(1)).postForEntity(anyString(), any(HttpEntity.class), eq(KeycloakTokenResponseDTO.class));
    }
    ...
    void getAccessToken_shouldUseRefreshToken_whenAccessTokenExpiredButRefreshTokenValid() {
        ...
        KeycloakTokenResponseDTO refreshedToken = new KeycloakTokenResponseDTO(
    ...
    void getAccessToken_shouldFetchNewToken_whenBothTokensExpired() {
        ...
        KeycloakTokenResponseDTO newToken = new KeycloakTokenResponseDTO(
            "brand-new-token", "brand-new-refresh", 300L, 1800L, "Bearer"
        );
        ...
        ReflectionTestUtils.setField(tokenService, "tokenExpiryTime", 
            java.time.Instant.now().minusSeconds(100));
        ReflectionTestUtils.setField(tokenService, "refreshTokenExpiryTime", 
            java.time.Instant.now().minusSeconds(100));
    ...
    void updateTokens_shouldSetDefaultRefreshExpiry_whenNotProvided() {
        ...
        java.time.Instant refreshExpiry = (java.time.Instant) ReflectionTestUtils.getField(tokenService, "refreshTokenExpiryTime");
    ...
    void updateTokens_shouldSetDefaultRefreshExpiry_whenZeroProvided() {
        KeycloakTokenResponseDTO tokenResponse = new KeycloakTokenResponseDTO(
            "test-access-token", "test-refresh-token", 300L, 0L, "Bearer"
        );
        when(restTemplate.postForEntity(anyString(), any(HttpEntity.class), eq(KeycloakTokenResponseDTO.class)))
            .thenReturn(new ResponseEntity<>(tokenResponse, HttpStatus.OK));
