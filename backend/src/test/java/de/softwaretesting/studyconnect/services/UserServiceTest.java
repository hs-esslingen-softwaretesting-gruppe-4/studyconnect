...
class UserServiceTest {
    ...
    void setUp() {
        ...
        testKeycloakUserResponseDTO = new KeycloakUserResponseDTO(
            "keycloak-uuid-123",
    ...
    void createUser_shouldCreateUserSuccessfully() {
        UserCreateRequestDTO createRequest = new UserCreateRequestDTO(
            "Max", "Mustermann", "Password123!", "test@example.com"
        );
        ...
        assertNotNull(response.getBody());
        assertEquals("test@example.com", response.getBody().getEmail());
        verify(keycloakService).createUserInRealm("Password123!", "test@example.com", "Max", "Mustermann");
        verify(userRepository).save(any(User.class));
    }
    ...
    void createUser_shouldSetKeycloakUUID() {
        ...
        verify(userRepository).save(argThat(user -> 
            "keycloak-uuid-123".equals(user.getKeycloakUUID())
        ));
    }
    ...
    void createAdmin_shouldCreateAdminSuccessfully() {
        UserCreateRequestDTO createRequest = new UserCreateRequestDTO(
            "Admin", "User", "AdminPass123!", "admin@example.com"
        );

        KeycloakUserResponseDTO adminKeycloakUser = new KeycloakUserResponseDTO(
            "admin-keycloak-uuid",
            System.currentTimeMillis(),
            "admin@example.com",
            true,
        ...
        when(keycloakService.createAdminUserInRealm(anyString(), anyString(), anyString(), anyString()))
            .thenReturn(true);
        when(keycloakService.retrieveUserByEmail("admin@example.com"))
            .thenReturn(adminKeycloakUser);
        when(userRepository.save(any(User.class))).thenAnswer(inv -> inv.getArgument(0));
        when(userResponseMapper.toDto(any(User.class))).thenReturn(
            new UserResponseDTO(2L, "admin@example.com", "User", "Admin")
        );

        ResponseEntity<UserResponseDTO> response = userService.createAdmin(createRequest);

        assertEquals(HttpStatus.CREATED, response.getStatusCode());
        verify(keycloakService).createAdminUserInRealm("AdminPass123!", "admin@example.com", "Admin", "User");
        verify(userRepository).save(any(User.class));
    }
    ...
    void init_shouldCreateRealmAndSyncUsers() {
        ...
        when(keycloakService.retrieveAllUsersInRealm()).thenReturn(List.of(testKeycloakUserResponseDTO));
